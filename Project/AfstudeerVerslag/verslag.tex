...
Binnen de opdracht waren er probleemsituaties onstaan die hieronder
...
Zie voor het volledige plan van aanpak bijlage 1. Het doel van het 
...
Er zal gebruik gemaakt worden van  SCRUM, een ontwikkelmethode, welke flexibel is en resulteert in een product dat  veel
...
gebruikt worden, waarin de documenten geschreven en opgeslagen worden.
...
gebruikt gaan worden. Welk platform,  PlugTop (een kleine computer server voor gebruik in huis / kantoor)  en  
protocollen  er  gebruikt gaan worden. Daarna wordt de PlugTop opgezet samen met een  mobiel  apparaat
...
webinterface. In deze webinterface kan men de code bekijken. Tevens kan de git url
...
om meer ervaring op te doen met \LaTeX\space. Het afstudeerverslag leek hen de uitgelezen kans hiervoor.
...
Tevens wordt gebruik gemaakt van een Eclipse plugin genaamd Colorer[5].
...
%\end{wrapfigure}
Om te kunnen werken met encryptie en TLS in native Java code,
wordt er gebruik gemaakt van Bouncy Castle wat een gratis open-source software is.
Bouncy Castle is een bibliotheek die binnen het project gebruikt
wordt om onder andere nieuwe certificaten te kunnen genereren vanuit Java code, alsmede een CSR
(Certificate Signing Request) bestand uit te lezen en deze gegevens te gebruiken om een nieuw certificaat te
ondertekenen door middel van een CA (Certificate Authority). In dit CSR bestand
...
aangemaakt door middel van Bouncy Castle. Onder Android wordt er ook gebruik
gemaakt van de Bouncy Castle bibliotheek, echter heet dat onder Android Spongy
Castle, maar het is exact dezelfde code.
...
met een globaal onderzoek naar het domotica veld en naar PlugTop computers. Het
...
authenticeren bij een server. Een certificaat wordt gegenereerd op de manier die te
zien is in figuur \ref{ssl_ad_1}. In een certificaat moeten gegevens staan waarmee
de client te identificeren is. Als er een aanvraag wordt gedaan, moet de
beheerder kunnen zien van wie deze aanvraag komt. Er is in het project
...
Er ontstond echter een probleem. TomCat leest namelijk de truststore tijdens het opstarten
...
\\Om dat probleem op te lossen, dat de TomCat server niet
telkens opnieuw herstart hoeft te worden, is ervoor gekozen om een eigen
...
om gebruik te maken van de cache gegevens uit de database, om zodoende de administrator te
kunnen valideren.
...
gebruikersnaam en wachtwoord die overeen moeten komen met een bestaand OpenRemote
...
Dit configuratiescherm en het user managementscherm zijn beide beveiligd met
...
Certificaat. Er wordt, als er verbinding is, gecontroleert of het client certificaat bekend
...
\begin{longtable}{|| l | l ||}\hline
    ...
    TrustStore       & Zie: KeyStore, een TrustStore bevat certificaten     \\
                     ...
                     & Of een CA certificaat waarbij de ondertekende        \\
